...
class OpencVino_DeepSearchAgent:
    ...
    def analyze(self, modality, folder_path, query):
        """
        ...
        if modality.lower() == 'document':
            ...
            engine = Image_vector_store(collection_name='Image_search',folder_path=folder_path)
            engine.creating_vector_store()
            captions = engine.retrieval()
            ...
            engine.response()
            self.context_data = engine.retrival(query="What is the video about? ")+""
